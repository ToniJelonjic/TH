...
const TablicaUređaj = (props) => {
  console.log(props, "tablica");
  return (
    <div className="table-style">
      {/* <div>
        {props.data.map((data) => {
          if (
            data.grupaid === props.groupValue &&
            data.podgrupaid === props.subGroupValue
          ) {
            return <div>{data.naziv}</div>;
          }
        })}
      </div> */}
              ...
              <tr key={data.id}>
                ...
                <td
                  className={`device-table-info font ${
                    data.active ? "active-user" : "non-active-user"
                  }`}
                >
                  {data.active ? "Aktivan" : "Neaktivan"}
                </td>
              </tr>
            );
          })}
